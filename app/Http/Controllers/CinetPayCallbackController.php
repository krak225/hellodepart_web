		  ...
		  dd('err: '.$err);
			...
			if(!empty($checkout_session)){
				...
				if($status == 'ACCEPTED'){
					...
					return redirect()->route('consulter.reservation');
			...
			try {
				...
				if(!empty($checkout_session)){
					...
					if ($code == '00' && $amount == $checkout_session->amount) {
						...
						//Récupérer le client qui à la facture
						$client = Client::find($checkout_session->client_id);
						...
						if($client->$client_email !=null){
							...
							$message->to($email_data['email'] ,$email_data['nom'] ,$email_data['prenoms'])
								->subject('Reçu de paiement de réservation de ticket de voyage')
